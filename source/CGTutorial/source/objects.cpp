...
static void createCube()
{
	...
	// Our vertices. Tree consecutive floats give a 3D vertex; Three consecutive vertices give a triangle.
	// A cube has 6 faces with 2 triangles each, so this makes 6*2=12 triangles, and 12*3 vertices
	// Jeweils 3 Werte sind Punkt (x,y,z).
	...
	glGenBuffers(1, &vertexbuffer);
	glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
    // Die in Speicher gelegten eckpunkte der dreiecke werden eingetragen. (siehe Mitschriften-Skizze Buffer 327 mit wŸrfel)
	glBufferData(GL_ARRAY_BUFFER, sizeof(g_vertex_buffer_data), g_vertex_buffer_data, GL_STATIC_DRAW);
	...
	// Beschreiben, dass es einen anknŸpfungspunkt zum Vertex-shader gibt, und dass an die punkte kommen
    glEnableVertexAttribArray(0); // Kein Disable ausführen !
	glBindBuffer(GL_ARRAY_BUFFER, vertexbuffer);
	glVertexAttribPointer(
			0,                  // attribute. No particular reason for 0, but must match the layout in the shader.
			3,                  // size // wie viele gehen aufeinmal rŸber
			GL_FLOAT,           // type
			GL_FALSE,           // normalized?
			0,                  // stride
			(void*)0            // array buffer offset
...
void drawCube()
{
    // if, da VertexArrayIDSolidCube am anfang noch nicht existiert.
	if (!VertexArrayIDSolidCube)
	{
		createCube();
	}

	// Draw the triangles ! (12 dreiecke mit jew. 3 Punkten
    // nimm die daten aus VertexArrayIDSolidCube (bekommt id des containers,von dem sich der Vertex shader die Daten holt.)
	glBindVertexArray(VertexArrayIDSolidCube);
